...
echo ExplainQueries();
CleanTable();
echo "<h3>Preparing queries 1 - 5 </h3>";
   ...
   $query5=Query5();
   $intypes=array('text');
   $outtypes=array('integer');
   $outtypesfloat=array('float','float');
   // for a select prepare the second param is an array of the types of 
   // the input ? types and the third an array of the types of the output fields
   // for query1 the input ? is a string -- location -- and the output is an 
   // integer num_emp
   $pquery1 = $con->prepare($query1,$intypes,$outtypes);
   error_check($pquery1,'preparing query1');
   $pquery2 = $con->prepare($query2,$intypes,$outtypes);
   error_check($pquery2,'preparing query2');
   $pquery3 = $con->prepare($query3,$intypes,$outtypes);
   error_check($pquery3,'preparing query3');
   // for query 4 the output fields are two floats, tot_hours and tot_cost
   $pquery4 = $con->prepare($query4,$intypes,$outtypesfloat);
   error_check($pquery4,'preparing query4');
   $pquery5 = $con->prepare($query5,$intypes,$outtypes);
   error_check($pquery5,'preparing query5');
$query0=Query0();
...
while( is_array(   $location = $outer->fetchRow(MDB2_FETCHMODE_ASSOC))){
   $loc = $location['plocation'];
   echo "<h3>Retrieving results for location $loc</h3>";
   echo ShortExp('num_emp',$query1,$loc);
   $res = $pquery1->execute($loc);
   error_check($res,"num_emp for $loc");
   $row = $res->fetchRow();
   $num_emp = $row[0];
   echo "Retrieved $num_emp<br>";
   echo ShortExp('num_proj',$query2,$loc);
   $res =  $pquery2->execute($loc);
   error_check($res,"num_proj for $loc");
   $row = $res->fetchRow();
   $num_proj = $row[0];
   echo "Retrieved $num_proj<br>";
   echo ShortExp('num_work',$query3,$loc);
   $res =  $pquery3->execute($loc);
   error_check($res,"num_work for $loc");
   $row = $res->fetchRow();
   $num_work = $row[0];
   echo "Retrieved $num_work<br>";
//Escape("After num_work  execute");
   echo ShortExp('tot_hours and tot_cost',$query4,$loc);
   $res =  $pquery4->execute($loc);
   error_check($res,"tot_hours and tot_cost for $loc");
   $row = $res->fetchRow();
   $tot_hours = $row[0];
   $tot_cost = $row[1];
   echo "Retrieved $tot_hours and $tot_cost<br>";
   echo ShortExp('num_dept',$query5,$loc);
   $res =  $pquery5->execute($loc);
   error_check($res,"num_dept for $loc");
   $row = $res->fetchRow();
   $num_dept = $row[0];
   ...
   # important to free memory used by $outer and prepared statements
$outer->free();
$pquery1->free();
$pquery2->free();
$pquery3->free();
...
<br>
In this version prepared statements are used.  These are prepared outside the main loop
with a placeholder <b>?</b> used for the location.  They are executed inside the main loop
with the location specified to a particular location for each iteration of the loop.
</div>
HTML;
}
function CleanTable(){
...
function Query0a(){
   // this was just used for debugging.  ignore.
   return  <<<QUERY
   select distinct location plocation from m
QUERY;
}
...
function ShortExp($colname, $query,$val){
   return <<<HTML
   <p>Retrieving $colname with prepared query<pre class="query">$query</pre> specified to $val</p>
HTML;
}
function Query2(){
   return<<<QUERY
...
function Escape($message){
   ...
   $con->disconnect();
   echo "<div style='border:black thin solid'>Exited: $message</div>";
   echo "</body></html>";
   exit();
}
?>
